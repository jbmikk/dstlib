...
#define BMap_APPEND(KTYPE, VTYPE, UPPER, LOWER, BODY) \
S(BMapEntry, UPPER) *bmap_##LOWER##_append( \
	struct S(BMap, UPPER) *bmap, \
	KTYPE key, \
	VTYPE LOWER \
) BODY({ \
...
#define BMap_FUNCTIONS(KTYPE, VTYPE, UPPER, LOWER, BODY) \
	BMap_FUNCTION_LIST(_EXPAND_FUNCTION, KTYPE, VTYPE, UPPER, LOWER, BODY)
...
#define BMap_DEFINE(KTYPE, VTYPE, UPPER, LOWER) \
	...
	BMap_FUNCTIONS(KTYPE, VTYPE, UPPER, LOWER, _PROTOTYPE)
