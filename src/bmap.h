...
S(BMapEntry, UPPER) *bmap_##LOWER##_m_get( \
	struct S(BMap, UPPER) *bmap, \
	KTYPE key \
) BODY({ \
	BMapComparator cmp; \
	COMPARATOR_INIT(cmp, key); \
	return (S(BMapEntry, UPPER) *)bmap_m_get( \
		&bmap->bmap, \
		sizeof(struct S(BMapEntry, UPPER)), \
		&cmp \
	); \
...
#define BMap_M_GET_AT(KTYPE, VTYPE, UPPER, LOWER, BODY) \
S(BMapEntry, UPPER) *bmap_##LOWER##_m_get_at( \
	struct S(BMap, UPPER) *bmap, \
	KTYPE key, \
	int index \
) BODY({ \
	BMapComparator cmp; \
	COMPARATOR_INIT(cmp, key); \
	return (S(BMapEntry, UPPER) *)bmap_m_get_at( \
		&bmap->bmap, \
		sizeof(struct S(BMapEntry, UPPER)), \
		&cmp, \
		index \
...
#define BMap_M_GET_LAST(KTYPE, VTYPE, UPPER, LOWER, BODY) \
S(BMapEntry, UPPER) *bmap_##LOWER##_m_get_last( \
	struct S(BMap, UPPER) *bmap, \
	KTYPE key \
) BODY({ \
	BMapComparator cmp; \
	COMPARATOR_INIT(cmp, key); \
	return (S(BMapEntry, UPPER) *)bmap_m_get_last( \
		&bmap->bmap, \
		sizeof(struct S(BMapEntry, UPPER)), \
		&cmp \
	); \
...
#define BMap_FUNCTION_LIST(_, ...) \
	...
	_(BMap_M_GET, __VA_ARGS__) \
	_(BMap_M_GET_AT, __VA_ARGS__) \
	_(BMap_M_GET_LAST, __VA_ARGS__) \
