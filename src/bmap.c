 ...
 */
static void _scan_next(struct BMapScan *scan, BMap *bmap, unsigned int size, BMapComparator *cmp)
{
...
BMapEntry *bmap_append(BMap *bmap, unsigned int size, BMapComparator *cmp)
{
	struct BMapScan scan = {NULL, NULL, NULL};
	_scan(&scan, bmap, size, cmp);
	_scan_next(&scan, bmap, size, cmp);

	return _prepend(bmap, size, scan.equal);
}
